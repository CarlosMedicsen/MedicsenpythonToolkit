...
def SimpleFeedback() -> None:
    ...
    # Configuración del PID
    kP = 0.1 #Si hay 1W de diferenciam, el PID subirá 0.1V
    kI = 0.01
    kD = 0

    PowerRef = 8 #W

    pid = PID(kP=kP, kI=kI, kD=kD, setpoint=PowerRef, max_output=0.8)
    ...
    while time.time() < Texp:
        ...
        data = data.append({'Tiempo (s)': dt, 'Potencia (W)': P, 'Salida PID (V)': output}, ignore_index=True)
    pulser.output_off()
    ...
    plt.xlabel('Tiempo (s)')
    plt.ylabel('Valor')
    plt.title('Control de Potencia con PID')
    plt.legend()
    plt.grid()
    plt.show()
